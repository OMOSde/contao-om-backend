...
class BackendLinks extends \Backend
{
    ...
    protected function addBackendLinksTop($strContent)
    {
        ...
        {
            $strStyle = ($link->icon) ? sprintf('background:url(%s) left 13px no-repeat;', \FilesModel::findByUuid($link->icon)->path) : sprintf('display:inline-block;margin-left:18px;padding: 17px 10px 17px 10px;');
